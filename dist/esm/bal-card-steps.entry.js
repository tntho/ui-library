...
const CardSteps = class {
    ...
    render() {
        return (h(Host, null, h("div", { class: ['tabs', this.inverted ? 'is-inverted' : '', this.hidden ? 'is-hidden' : ''].join(' ') }, h("ul", null, this.stepOptions.filter(step => !step.hidden && !this.hidden).map((step, index) => (h("li", { class: [
                step.active ? 'is-active' : '',
                step.disabled ? 'is-disabled' : '',
                step.done ? 'is-done' : '',
            ].join(' ') }, h("a", { onClick: () => this.onSelectStep(step), title: step.label }, h("span", { class: "step-index" }, h("span", null, index + 1)))))))), h("slot", null)));
    }
