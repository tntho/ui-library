...
const FileUpload = class {
        ...
        if (!this.disabled) {
            ...
            for (let index = 0; index < files.length; index++) {
                ...
                if (this.accept &&
                    ...
                    rejectReasons.push(fileUpload_type.FileUploadRejectionReason.BAD_EXTENSION);
                }
                if (this.maxFileSize && file.size > this.maxFileSize) {
                    rejectReasons.push(fileUpload_type.FileUploadRejectionReason.FILE_TOO_BIG);
                }
                ...
                if (this.maxBundleSize && bundleSize > this.maxBundleSize) {
                    rejectReasons.push(fileUpload_type.FileUploadRejectionReason.FILE_SIZE_SUM_TOO_BIG);
                }
                if (this.maxFiles && list.length + 1 > this.maxFiles) {
                    rejectReasons.push(fileUpload_type.FileUploadRejectionReason.TOO_MANY_FILES);
                }
                const duplicatedFiles = list.filter((f) => f.size === file.size &&
                    f.name === file.name &&
                    f.type === file.type);
                if (duplicatedFiles.length > 0) {
                    rejectReasons.push(fileUpload_type.FileUploadRejectionReason.DUPLICATED_FILE);
                }
    ...
    render() {
        return (index.h(index.Host, { class: ["bal-file-upload", this.disabled ? "is-disabled" : ""].join(" ") }, index.h("div", { class: "file" }, index.h("label", { class: [
                "file-label",
                this.isOver ? "is-hovered" : "",
                this.disabled ? "is-disabled" : "",
            ].join(" ") }, index.h("input", { class: "file-input", type: "file", name: "resume", multiple: this.multiple, disabled: this.disabled, accept: this.accept, onChange: () => this.handleFiles(this.fileInput.files), ref: (el) => (this.fileInput = el) }), index.h("span", { class: "file-cta" }, index.h("span", { class: "file-icon" }, index.h("bal-icon", { name: "upload", size: "medium" })), index.h("span", { class: "file-label" }, this.label)))), index.h("bal-list", { disabled: true, border: true }, this.files.map((file, index$1) => (index.h("bal-list-item", null, index.h("bal-list-item-icon", null, index.h("bal-icon", { size: "medium", name: "document" })), index.h("bal-list-item-content", null, index.h("bal-list-item-title", null, file.name), index.h("bal-list-item-subtitle", null, filesize(file.size))), index.h("bal-list-item-icon", { right: true, class: "file-remove", onClick: () => this.removeFile(index$1) }, index.h("bal-icon", { size: "medium", name: "trash" }))))))));
    }
